...
import { getStaffNotes, createStaffNote } from "@/services/staff.service";

const schema = z.object({ body: z.string().min(1).max(1000) });

export const GET = withErrorHandling(async (req: NextRequest) => {
  const payload = await requireRole(req, "STAFF", "OWNER");
  return ok(await getStaffNotes(payload.userId));
});
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  return created(await createStaffNote(payload.userId, parsed.data.body));
});
